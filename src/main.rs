...
fn alignment(args: State<Vec<String>>, chromosome: String, from: u32, to: u32) -> Json<Vec<AlignmentNucleobase>> {
    Json(get_reads(Path::new(&args[1].clone()), Path::new(&args[2].clone()) , chromosome, from, to))
}
#[get("/variant/<chromosome>/<from>/<to>")]
fn variant(args: State<Vec<String>>, chromosome: String, from: u32, to: u32) -> Json<Vec<Variant>> {
